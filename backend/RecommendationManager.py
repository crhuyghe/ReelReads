...
class RecommendationManager:
    ...
    def get_recommendations(self, user_vector, user_history, top_n=10):
        ...
        joined_movies = joined_movies.reset_index(drop=True)

        movies = pd.read_csv("database/movie_data/movies_metadata_trimmed.csv", usecols=["id", "original_title"])

        print(pd.merge(joined_movies.head(top_n), movies, on="id", how="inner")[["sim", "avg_rating", "score", "original_title"]])

        # print(movies.loc[movies["id"].isin(set(joined_movies.head(top_n)["id"])), ("original_title", "id")])

        return joined_movies.head(top_n)
    ...
    def _get_ratings(self):
        ...
        return None, pd.concat((pd.read_csv("database/movie_data/als_movie_ratings1.csv"), pd.read_csv("database/movie_data/als_movie_ratings2.csv")))
